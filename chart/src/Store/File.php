...
class File {
    ...
    function login($uId, $info) {
        file_put_contents($this->online_dir . $uId, serialize($info));
        file_put_contents($this->online_dir . 'FD_' . $info['fd'], serialize($info));
    }

    function logout($uId) {
        $user = $this->getUser($uId);
        if ($user) {
            unlink($this->online_dir . 'FD_' . $user['fd']);
        }
        unlink($this->online_dir . $uId);
    }

    function getOnlineUsers() {
        $files = scandir($this->online_dir);
    ...
    function getUserFd($fd) {
        if (!is_file($this->online_dir . 'FD_' . $fd)) {
            return false;
        }
        $ret = file_get_contents($this->online_dir . 'FD_' . $fd);
