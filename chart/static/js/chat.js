...
function listenEvent() {
    ...
    ws.onmessage = function (e) {
        ...
        if (cmd == 'login') {
            ...
            uId = $.evalJSON(e.data).uId;
            //获取在线列表
            ws.send($.toJSON({cmd : 'getOnline', 'uId' : uId}));
            //获取历史记录
            ws.send($.toJSON({cmd : 'getHistory', 'uId' : uId}));
            //alert( "收到消息了:"+e.data );
...
function showOnlineList(dataObj) {
    ...
    for (var i = 0; i < dataObj.list.length; i++) {
        li = li + "<li id='inroom_" + dataObj.list[i].uId + "'>" +
        "<a href=\"javascript:selectUser('"
        + dataObj.list[i].uId + "')\">" + "<img src='" + dataObj.list[i].avatar
        + "' title='" + dataObj.list[i].name + "' width='50' height='50'></a></li>";

        userlist[dataObj.list[i].uId] = dataObj.list[i].name;

        if (dataObj.list[i].uId != uId) {
            option = option + "<option value='" + dataObj.list[i].uId + "' id='user_" + dataObj.list[i].uId + "'>"
...
function showNewUser(dataObj) {
    if (!userlist[dataObj.uId]) {
        userlist[dataObj.uId] = dataObj.name;
        if (dataObj.uId != uId) {
            $('#userlist').append("<option value='" + dataObj.uId + "' id='user_" + dataObj.uId + "'>" + dataObj.name + "</option>");

        }
        $('#left-userlist').append(
            "<li id='inroom_" + dataObj.uId + "'>" +
                '<a href="javascript: selectUser(\'' + dataObj.uId + '\')">' + "<img src='" + dataObj.avatar
...
function showNewMsg(dataObj) {
    ...
    if (fromId == 0) {
        ...
        if (channal == 3) {
            if (to) {
                if (uId == fromId) {
                    said = "我悄悄的对" + dataObj.toName + "说:"
                } else {
                    said = dataObj.user.name + "悄悄的对我说:"
                }
            } else {
                if (fromId == uId) {
                    said = '我对大家说:';
                } else {
                    said = dataObj.user.name + '对大家说:';
                }
            }

            html += '<span style="color: green">【历史记录】</span><span style="color: orange">' + said;
            html += '</span>';
        } else {
            //如果说话的是我自己
            if (uId == fromId) {
